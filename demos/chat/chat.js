// Create a node
var browserId = localStorage.browserId || 'B-' + Math.random().toString(36).slice(2)
var escapedId = JSON.stringify(browserId)
...
let createListeners = function () {
    ...
    function submit() {
        ...
        node.setPatch('/chat', `[-0:-0] = ${messageBody}`)
        resetText()
        sendbox.value = ""
    ...
    function setNotTyping() {
        ...
        if (users[browserId].typing)
            node.setPatch('/usr', `[${escapedId}].typing = false`)
    ...
    function updateTyping() {
        ...
        if (typing && lastCheck != check) {
            node.setPatch('/usr', `[${escapedId}].typing = ${JSON.stringify(check)}`)
